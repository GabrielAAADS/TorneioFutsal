...
export default function Torneios() {
  ...
  const handleSelecionarTorneio = async (id: string) => {
    try {
      ...
      setTorneioSelecionado(response.data);
        ...
        {torneios.map((torneio) => (
          <li key={torneio.id} className="border p-2 flex justify-between">
            ...
            <div>
              ...
              <button onClick={() => excluirTorneio(torneio.id)} className="bg-red-500 text-white p-1">Excluir</button>
            </div>
          </li>
        ))}
      </ul>
        ...
        <Dialog.Portal>
          ...
          <Dialog.Content className="fixed bg-white p-6 rounded shadow-md inset-1/3">
            ...
            {loadingEquipes ? (
              ...
              <Equipes idTorneio={torneioSelecionado.id} onEquipeAlterada={carregarTorneios}/>
            ) : (
